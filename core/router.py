...
class UniversalProcessor:
    ...
    def _sync_to_dashboard(user_id, text, attachments):
        try:
            chat_file = "/home/user/BANE_CORE/dashboard/chat_history.json"
            history = []
    ...
    async def _wait_loop(platform, send_reply, send_buttons, is_private, original_text):
        ...
        while waited < max_wait:
            ...
            # === AUTO-SUDO: Detect password prompts and inject ===
            try:
                from security.detector import detect_password_prompt, auto_inject_sudo
                if detect_password_prompt():
                    logger.info("🔐 Password prompt detected — auto-injecting sudo password")
                    auto_inject_sudo()
                    waited = 0
                    await asyncio.sleep(2)
                    continue
            except Exception as e:
                logger.debug(f"Sudo detection check skipped: {e}")

            # === AUTO-APPROVE: Check for buttons (Accept/Allow/Always Run) ===
            buttons = detect_action_buttons()
            if buttons:
                from agent.turbo import get_automation_state
                is_turbo = get_automation_state()["autonomous"]
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # === HYBRID MODE: AUTO-DECRYPT INCOMING ===
    # If user sends encrypted text but wants plain replies, "Middleman" decrypts here.
    if message_text:
        message_text = decrypt_text(message_text)
...
async def handle_messenger_message(payload: dict, recipient_id: str, access_token: str):
    ...
    # Normalize text if somehow missed
    if text:
         text = decrypt_text(text)
    ...
    role = "admin" if str(sender_id) == os.getenv("MESSENGER_ADMIN_ID", "MESSENGER_USER_ID") else "guest"
    ...
    try:
        ...
        mid_file = "/home/user/BANE_CORE/storage/last_user_mids.json"
        data = {}
