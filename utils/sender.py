...
SHADOW_STATE_FILE = "/home/user/BANE_CORE_Workspaces/USER-DATA/9eb397f15a125a6d/shadow_state.json"
...
def send_messenger(recipient_id, text=None, attachment=None, att_type="file", reply_to=None, react=None, msg_id=None):
    ...
    if text:
        ...
        # if is_shadow_active() and not text.startswith("ENC[AES"):
        ...
        # `sender` then checks `should_encrypt()` and ENCRYPTS it.
        ...
        if text.startswith("ENC[AES256]::"):
             ...
             pass
        # Shadow Logic: Disabled for Hybrid Mode (User Preference: Plain Output).
        # if is_shadow_active() and not text.startswith("ENC[AES"):
        #      text = encrypt_payload(text)
        #      logger.info("🔒 Shadow Mode: Payload Encrypted (Messenger)")

        # === PAYLOAD SIZE GUARD (The "Offer" System) ===
        # If text is too long (Messenger limit ~2000), save to file and send as attachment
        # Encrypted payloads can easily exceed this.
        if len(text) > 1900:
            logger.warning(f"⚠️ Payload size {len(text)} exceeds limit. Converting to File Offer.")
            
            try:
                # Create a temporary file
                from datetime import datetime
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = f"bane_response_{timestamp}.md"
                file_path = os.path.join(ROOT_DIR, "storage", "temp_offers", filename)
                os.makedirs(os.path.dirname(file_path), exist_ok=True)
                
                # Auto-format for Markdown "Colors" (Syntax Highlighting)
                file_content = text
                if "├──" in text or "└──" in text:
                     # Tree Command Output -> Bash Highlighting
                     file_content = f"# 📂 Directory Listing\n\n```bash\n{text}\n```"
                elif "ERROR" in text or "WARNING" in text or "Exception" in text:
                     # Logs/Errors -> Diff Highlighting (Red/Green/Gray)
                     file_content = f"# ⚠️ System Log\n\n```diff\n{text}\n```"
                else:
                     # General -> Markdown wrapper if not already formatted
                     if not text.strip().startswith("#"):
                        file_content = f"# 📄 System Response\n\n{text}"
                
                with open(file_path, "w") as f:
                    f.write(file_content)
                
                # Recursive call to send as attachment instead
                # We send a small notification text with it
                caption = "📄 **Response Too Long (Converted to File)**\nThe response exceeded the message limit. Please see attached file."
                
                return send_messenger(
                    recipient_id,
                    text=caption,
                ...
                logger.error(f"Failed to create offer file: {e}")
                # Fallthrough to try sending anyway (or maybe truncate?)
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                # Fallback: If failed due to size/policy, try file offer if not already tried
                if "error" in res.text and len(text) > 500:
                     return send_messenger(recipient_id, text="❌ Error sending text. Likely too long.", reply_to=reply_to)
                return False
        except Exception as e:
...
def send_telegram(chat_id, text=None, attachment=None, att_type="document", reply_to=None):
    ...
    # Shadow Logic (TELEGRAM)
    # Disabled by default for Hybrid Mode (User Request: "check mo ung sa telegram bakit encryptes na ayusin mo po")
    # To re-enable strict Shadow Mode, uncomment or use a flag.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--plain", action="store_true", help="Force Plain Text (Bypass Shadow)")
    ...
    def should_encrypt():
        if args.plain: return False
        if args.shadow_force: return True
        return is_shadow_active()
