...
def test_security():
    ...
    test_paths = [
        ("/home/son/BANE_Workspaces/my_app/test.py", True),  # Allowed workspace
        ("/home/son/BANE/main.py", False),                # Core file (blocked)
        ("/home/son/BANE/core/router.py", False),          # Core dir (blocked)
        ("/tmp/some_log.txt", True),                           # Temp (allowed)
        ("/home/son/BANE/storage/test.png", True),         # Storage (allowed)
    ...
    m_file = "/home/son/BANE/.maintenance"
    try:
        open(m_file, 'a').close()
        print(f"  Maintenance mode ON: {is_maintenance_mode()} (Expected: True)")
        print(f"  Path /home/son/BANE/main.py allowed? {is_allowed_path('/home/son/BANE/main.py')} (Expected: True)")
        os.remove(m_file)
